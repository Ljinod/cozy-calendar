...
module.exports.cancelShare = function(event, callback) {
  if (event.shareID) {
    return Sharing.exists(event.shareID, function(err, exists) {
      if (exists) {
        return cozydb.api.revokeSharingFromSharer(event.shareID, function(err, body) {
          if (err != null) {
            return callback(err);
          } else {
            return callback();
        ...
        return Sharing.request('byShareID', {
          key: event.shareID
        }, function(err, docSharing) {
          return cozydb.api.revokeSharingFromRecipient(docSharing[0]._id, function(err, body) {
            if (err != null) {
              return callback(err);
            } else {
              return callback();
